...
export type GeneratorTemperatureMapTile = Pick<GameTile, 'id' | 'base' | 'lithosphericData' | 'terrainData'> & {
...
export class GeneratorTemperatureMapService {
  public generateTemperature(map: GeneratorElevationMap): GeneratorTemperatureMap {
    ...
    map.forEach((tile, id) => {
      ...
      const elevationEffect = Math.max(0, tile.terrainData.elevation) * 0.0065; // Approx lapse rate: 6.5C per 1000m
